...
output/       テキスト表示とグラフ描画
tools/        検証用スタンドアロンスクリプト（厳密解ソルバー exact_bb.py、
              分野プロファイル検査 profile_check.py。本体からは参照されない）
main.py       エントリーポイント
```
...
アンケートで申告された興味のある分野を `interests` 行に書くと、レポートの
【興味のある分野】に相性と併記して表示される。
...
- 分野名は `config/fields.py` の名前と一致させる（項目名ではなく**分野名**。
  一致しない名前はエラーになる）。カンマ区切りで並べ、ダブルクォートで囲む。
- **省略できる**（行が無い／値が空＝申告なし。節ごと表示されない）。

詳細は `docs/requirements.md` 第12節。
